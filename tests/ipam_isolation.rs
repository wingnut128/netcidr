...
//! cross-tenant guarantees: cidr_blocks, allocations, audit log, idempotency
...
async fn create_cidr_block(app: &axum::Router, tenant: &str, cidr: &str) -> String {
    let r = send(
        app,
        "POST",
        "/ipam/cidr-blocks",
...
async fn cidr_blocks_are_isolated_per_tenant() {
    ...
    let s_a_id = create_cidr_block(&app, TENANT_A, "10.0.0.0/8").await;

    // B sees zero CIDR blocks.
    let r = send(&app, "GET", "/ipam/cidr-blocks", TENANT_B, None, None).await;
    assert_eq!(r.status, StatusCode::OK);
    assert_eq!(
        r.body["count"], 0,
        "tenant B must not see tenant A's cidr_block"
    );

    // A sees its own one cidr_block.
    let r = send(&app, "GET", "/ipam/cidr-blocks", TENANT_A, None, None).await;
    assert_eq!(r.status, StatusCode::OK);
    ...
    let r = send(
        &app,
        "GET",
        &format!("/ipam/cidr-blocks/{s_a_id}"),
        TENANT_B,
        None,
        None,
    )
    ...
    assert_eq!(
        ...
        "cross-tenant cidr_block read must 404 (no existence leak)"
    );
}
...
async fn allocations_are_isolated_per_tenant() {
    ...
    let alloc = send(
        &app,
        "POST",
        &format!("/ipam/cidr-blocks/{s_a_id}/allocate-specific"),
        TENANT_A,
    ...
    // B trying to allocate inside A's cidr_block gets 404 (cidr_block hidden).
    let r = send(
        &app,
        "POST",
        &format!("/ipam/cidr-blocks/{s_a_id}/allocate-specific"),
    ...
    assert_eq!(
        r.status,
        StatusCode::NOT_FOUND,
        "cross-tenant allocation under A's cidr_block must 404"
    );
}
...
async fn audit_log_is_isolated_per_tenant() {
    let app = isolation_app().await;
    let _s_a_id = create_cidr_block(&app, TENANT_A, "10.0.0.0/8").await;
...
async fn idempotency_keys_are_isolated_per_tenant() {
    let app = isolation_app().await;
    let s_a_id = create_cidr_block(&app, TENANT_A, "10.0.0.0/8").await;
    let s_b_id = create_cidr_block(&app, TENANT_B, "10.0.0.0/8").await;
    ...
    // B uses the same key against its own cidr_block — must execute fresh.
    let r_b = send(
        &app,
        "POST",
        &format!("/ipam/cidr-blocks/{s_b_id}/allocate-specific"),
        TENANT_B,
        Some(r#"{"cidr":"10.2.0.0/16"}"#),
        Some(key),
    )
